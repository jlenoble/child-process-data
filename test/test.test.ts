...
describe("Testing childProcessData", () => {
  const echo = (...args) => {
  ...
  it(`childProcessData can test buffered messages`, () => {
    return Promise.all([
  ...
  it(`childProcessData can multi-test buffered messages`, () => {
    return Promise.all([
      echo("Little silly message").then(res => {
        function make(str) {
  ...
  it(`childProcessData can test messages up to a point`, () => {
    return Promise.all([
  ...
  it(`childProcessData can multi-test messages up to a point`, () => {
    return Promise.all([
      node("./test/examples/normal-exit.js").then(res => {
        function make(str) {
          return msg => !msg.includes(str);
  ...
  it(`childProcessData can multi-test messages up to a point, included`, () => {
    return Promise.all([
      node("./test/examples/normal-exit.js").then(res => {
        ...
        expect(res.all()).to.equal("lorem\nipsum\ndolor\nsit\namet\n");
        expect(res.testUpTo([...good()], "dolor", { included: true })).to.be
          .true;
        expect(res.testUpTo([...bad()], "dolor", { included: true })).to.be
          .false;
      })
    ]);
  });

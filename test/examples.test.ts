...
describe("Testing README.md examples", () => {
  it(`Testing 'Simple usage' example`, () => {
    const proc1 = spawn("echo", ["Lorem ipsum"]); // Spawns a child process
    const proc2 = spawn("echo", ["dolor sit amet"]); // Spawns another child
    // process

    return Promise.all([
      childProcessData(proc1).then(res => {
        // Recovers output from proc1
        expect(res.all()).to.equal("Lorem ipsum\n");
      }),
      childProcessData(proc2).then(res => {
        // Recovers output from proc2
        expect(res.all()).to.equal("dolor sit amet\n");
      })
    ]);
  });

  it(`Testing 'Accessing individual messages' example`, () => {
  ...
  it(`Testing 'Accessing all messages' example`, () => {
    return childProcessData(
      spawn("node", ["./test/examples/normal-exit.js"])
    ).then(res => {
  ...
  it(`Testing 'Accessing uncaught error messages' example`, () => {
    return childProcessData(
      spawn("node", ["./test/examples/error-exit.js"])
    ).catch(_err => {
      const res = _err.result;
